...
/**
 * A convenience class from which to extend all non-visual AbstractBeans. It manages the
 * PropertyChange notification system, making it relatively trivial to add support for property
 * change events in getters/setters.
 *
 * <p>A non-visual java bean is a Java class that conforms to the AbstractBean patterns to allow
 * visual manipulation of the bean's properties and event handlers at design-time.
 *
 * <p>Here is a simple example bean that contains one property, foo, and the proper pattern for
 ...
 * </pre>
 *
 * <p>You will notice that "getFoo()" is used in the setFoo method rather than accessing "foo"
 * directly for the gets. This is done intentionally so that if a subclass overrides getFoo() to
 * return, for instance, a constant value the property change notification system will continue to
 * work properly.
 *
 * <p>The firePropertyChange method takes into account the old value and the new value. Only if the
 * two differ will it fire a property change event. So you can be assured from the above code
 * fragment that a property change event will only occur if old is indeed different from getFoo()
 *
 * <p><code>AbstractBean</code> also supports vetoable {@link PropertyChangeEvent} events. These
 * events are similar to <code>PropertyChange</code> events, except a special exception can be used
 * to veto changing the property. For example, perhaps the property is changing from "fred" to
 * "red", but a listener deems that "red" is unexceptable. In this case, the listener can fire a
 * veto exception and the property must remain "fred". For example:
 *
 * <pre>
 * <code>
 *  public class ABean extends AbstractBean {
 ...
 * </pre>
 *
 * <p>{@code AbstractBean} is not {@link java.io.Serializable}. Special care must be taken when
 * creating {@code Serializable} subclasses, as the {@code Serializable} listeners will not be
 * saved. Subclasses will need to manually save the serializable listeners. The {@link
 * AbstractSerializableBean} is {@code Serializable} and already handles the listeners correctly. If
 * possible, it is recommended that {@code Serializable} beans should extend {@code
 * AbstractSerializableBean}. If it is not possible, the {@code AbstractSerializableBean} bean
 * implementation provides details on how to correctly serialize an {@code AbstractBean} subclass.
 *
...
public abstract class AbstractBean {
  /**
   * Helper class that manages all the property change notification machinery. PropertyChangeSupport
   * cannot be extended directly because it requires a bean in the constructor, and the "this"
   * argument is not valid until after super construction. Hence, delegation instead of extension
   */
   ...
   * @return all of the <code>PropertyChangeListeners</code> associated with the named property. If
   *     no such listeners have been added, or if <code>propertyName</code> is null, an empty array
   *     is returned.
   */
   ...
   *
   * <p>This is merely a convenience wrapper around the more general firePropertyChange method that
  ...
  /**
   * Add a VetoableListener to the listener list. The listener is registered for all properties. The
   * same listener object may be added more than once, and will be called as many times as it is
  ...
  /**
   * Remove a VetoableChangeListener from the listener list. This removes a VetoableChangeListener
   * that was registered for all properties. If <code>listener</code> was added more than once to
   * the same event source, it will be notified one less time after being removed. If <code>listener
   * </code> is null, or was never added, no exception is thrown and no action is taken.
   *
  ...
  /**
   * Returns the list of VetoableChangeListeners. If named vetoable change listeners were added,
   * then VetoableChangeListenerProxy wrappers will returned
   *
   * <p>
   *
   * @return List of VetoableChangeListeners and VetoableChangeListenerProxys if named property
   *     change listeners were added.
   */
  ...
  /**
   * Add a VetoableChangeListener for a specific property. The listener will be invoked only when a
   * call on fireVetoableChange names that specific property. The same listener object may be added
   * more than once. For each property, the listener will be invoked the number of times it was
   * added for that property. If <code>propertyName</code> or <code>listener</code> is null, no
   * exception is thrown and no action is taken.
   *
  ...
  /**
   * Remove a VetoableChangeListener for a specific property. If <code>listener</code> was added
   * more than once to the same event source for the specified property, it will be notified one
   * less time after being removed. If <code>propertyName</code> is null, no exception is thrown and
   * no action is taken. If <code>listener</code> is null, or was never added for the specified
   * property, no exception is thrown and no action is taken.
   ...
   *
   * @param propertyName The name of the property being listened to
   * @return all the <code>VetoableChangeListeners</code> associated with the named property. If no
   *     such listeners have been added, or if <code>propertyName</code> is null, an empty array is
   *     returned.
   */
  ...
  /**
   * Report a vetoable property update to any registered listeners. If anyone vetos the change, then
   * fire a new event reverting everyone to the old value and then rethrow the
  ...
  /**
   * Fire a vetoable property update to any registered listeners. If anyone vetos the change, then
   * fire a new event reverting everyone to the old value and then rethrow the
   * PropertyVetoException.
   *
   * <p>No event is fired if old and new are equal and non-null.
