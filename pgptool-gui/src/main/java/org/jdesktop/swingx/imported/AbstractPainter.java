...
/**
 * A convenient base class from which concrete {@link Painter} implementations may extend. It
 * extends {@link org.jdesktop.beans.AbstractBean} as a convenience for adding property change
 * notification support. In addition, <code>AbstractPainter</code> provides subclasses with the
 * ability to cacheable painting operations, configure the drawing surface with common settings
 * (such as antialiasing and interpolation), and toggle whether a subclass paints or not via the
 * <code>visibility</code> property.
 *
 * <p>Subclasses of <code>AbstractPainter</code> generally need only override the {@link
 * #doPaint(Graphics2D, Object, int, int)} method. If a subclass requires more control over whether
 * cacheing is enabled, or for configuring the graphics state, then it may override the appropriate
 * protected methods to interpose its own behavior.
 *
 * <p>For example, here is the doPaint method of a simple <code>Painter</code> that paints an opaque
 * rectangle:
 *
 * <pre>
 * <code>
...
public abstract class AbstractPainter<T> extends AbstractBean implements Painter<T> {
  ...
  /**
   * A defensive copy of the Effects to apply to the results of the AbstractPainter's painting
   * operation. The array may be empty but it will never be null.
   *
  ...
  /**
   * A convenience method for specifying the filters to use based on BufferedImageOps. These will
   * each be individually wrapped by an ImageFilter and then setFilters(Effect... filters) will be
   * called with the resulting array
   *
  ...
  /**
   * Returns if antialiasing is turned on or not. The default value is true. This is a bound
   * property.
   *
  ...
  /**
   * Gets the current interpolation setting. This property determines if interpolation will be used
   * when drawing scaled images. @see java.awt.RenderingHints.KEY_INTERPOLATION.
   *
  ...
  /**
   * Sets a new value for the interpolation setting. This setting determines if interpolation should
  ...
  /**
   * Gets the visible property. This controls if the painter should paint itself. It is true by
   * default. Setting visible to false is good when you want to temporarily turn off a painter. An
   * example of this is a painter that you only use when a button is highlighted.
   *
  ...
  /**
   * Sets the visible property. This controls if the painter should paint itself. It is true by
   * default. Setting visible to false is good when you want to temporarily turn off a painter. An
   * example of this is a painter that you only use when a button is highlighted.
   *
  ...
  /**
   * Gets whether this <code>AbstractPainter</code> can be cached as an image. If cacheing is
   * enabled, then it is the responsibility of the developer to invalidate the painter (via {@link
   * #clearCache}) if external state has changed in such a way that the painter is invalidated and
  ...
  /**
   * Sets whether this <code>AbstractPainter</code> can be cached as an image. If true, this is
   * treated as a hint. That is, a cacheable may or may not be used. The {@link #shouldUseCache}
   * method actually determines whether the cacheable is used. However, if false, then this is
   * treated as an absolute value. That is, no cacheable will be used.
   *
   * <p>If set to false, then #clearCache is called to free system resources.
   *
   * @param cacheable
   */
  ...
  /**
   * Call this method to clear the cacheable. This may be called whether there is a cacheable being
   * used or not. If cleared, on the next call to <code>paint</code>, the painting routines will be
   * called.
   *
   * <p><strong>Subclasses</strong>If overridden in subclasses, you <strong>must</strong> call
   * super.clearCache, or physical resources (such as an Image) may leak.
   */
  ...
  /**
   * Only made package private for testing. Don't call this method outside of this class! This is
  ...
  /**
   * Called to allow <code>Painter</code> subclasses a chance to see if any state in the given
   * object has changed from the last paint operation. If it has, then the <code>Painter</code> has
   * a chance to mark itself as dirty, thus causing a repaint, even if cached.
   *
   * @param object
   */
  ...
  /**
   * Ye olde dirty bit. If true, then the painter is considered dirty and in need of being
   * repainted. This is a bound property.
   *
  ...
  /**
   * Sets the dirty bit. If true, then the painter is considered dirty, and the cache will be
   * cleared. This property is bound.
   *
  ...
  /**
   * Returns true if the painter should use caching. This method allows subclasses to specify the
   * heuristics regarding whether to cache or not. If a <code>Painter</code> has intelligent rules
   * regarding painting times, and can more accurately indicate whether it should be cached, it
   * could implement that logic in this method.
   *
   * @return whether or not a cache should be used
   */
  protected boolean shouldUseCache() {
    return isCacheable()
        || filters.length > 0; // NOTE, I can only do this because getFilters() is final
  }

  /**
   * This method is called by the <code>paint</code> method prior to any drawing operations to
   * configure the drawing surface. The default implementation sets the rendering hints that have
   * been specified for this <code>AbstractPainter</code>.
   *
   * <p>This method can be overridden by subclasses to modify the drawing surface before any
   * painting happens.
   *
  ...
  public final void paint(Graphics2D g, T obj, int width, int height) {
    ...
    if (shouldUseCache() || filters.length > 0) {
      ...
      boolean invalidCache =
          null == cache || cache.getWidth() != width || cache.getHeight() != height;
