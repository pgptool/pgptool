...
/**
 * <p>
 * A {@link Painter} implementation composed of an array of
 * <code>Painter</code>s. <code>CompoundPainter</code> provides a means for
 * combining several individual <code>Painter</code>s, or groups of them, into
 * one logical unit. Each of the <code>Painter</code>s are executed in order.
 * BufferedImageOp filter effects can be applied to them together as a whole.
 * The entire set of painting operations may be cached together.
 * </p>
 *
 * <p>
 * </p>
...
public class CompoundPainter<T> extends AbstractPainter<T> {
	...
	/**
	 * Sets the array of Painters to use. These painters will be executed in order.
	 * A null value will be treated as an empty array. To prevent unexpected
	 ...
	 * clipPreserved to true can be used to let one painter mask other painters that
	 * come after it.
	 * 
	...
	/**
	 * Sets if the clip should be preserved. Normally the clip will be reset between
	 * each painter. Setting clipPreserved to true can be used to let one painter
	 * mask other painters that come after it.
	 * 
	...
	/**
	 * Used by {@link #isDirty()} to check if the child <code>Painter</code>s should
	 * be checked for their <code>dirty</code> flag as part of processing.<br>
	 * Default value is: <code>true</code><br>
	 * This should be set to </code>false</code> if the cacheable state of the child
	 * <code>Painter</code>s are different from each other. This will allow the
	 * cacheable == <code>true</code> <code>Painter</code>s to keep their cached
	 * image during regular repaints. In this case, client code should call
	 ...
	 * <code>Painter</code>s should be checked for their <code>dirty</code> flag as
	 * part of processing.
	 *
	...
	/**
	 * <p>
	 * This <code>CompoundPainter</code> is dirty if it, or (optionally) any of its
	 * children, are dirty. If the super implementation returns <code>true</code>,
	 * we return <code>true</code>. Otherwise, if
	 * {@link #isCheckingDirtyChildPainters()} is <code>true</code>, we iterate over
	 * all child <code>Painter</code>s and query them to see if they are dirty. If
	 * so, then <code>true</code> is returned. Otherwise, we return
	 * <code>false</code>.
	 * </p>
	 *
	 * {@inheritDoc} {@see #isCheckingDirtyChildPainters()}
	 */
	@Override
