...
/**
 * A {@link Painter} implementation composed of an array of <code>Painter</code>s. <code>
 * CompoundPainter</code> provides a means for combining several individual <code>Painter</code>s,
 * or groups of them, into one logical unit. Each of the <code>Painter</code>s are executed in
 * order. BufferedImageOp filter effects can be applied to them together as a whole. The entire set
 * of painting operations may be cached together.
 *
 * <p>
 *
 * <p>For example, if I want to create a CompoundPainter that started with a blue background, had
 * pinstripes on it running at a 45 degree angle, and those pinstripes appeared to "fade in" from
 * left to right, I would write the following:
 *
 * <pre>
 * <code>
...
public class CompoundPainter<T> extends AbstractPainter<T> {
   ...
   * copy of the given array is made, so that future modification to the array does not result in
   * changes to the CompoundPainter.
   *
  ...
  /**
   * Indicates if the clip produced by any painter is left set once it finishes painting. Normally
   * the clip will be reset between each painter. Setting clipPreserved to true can be used to let
  ...
  /**
   * Sets if the clip should be preserved. Normally the clip will be reset between each painter.
   * Setting clipPreserved to true can be used to let one painter mask other painters that come
   * after it.
   *
  ...
  /**
   * Iterates over all child <code>Painter</code>s and gives them a chance to validate themselves.
   * If any of the child painters are dirty, then this <code>CompoundPainter</code> marks itself as
   * dirty. {@inheritDoc}
   */
  @Override
   ...
   * their <code>dirty</code> flag as part of processing.<br>
   * Default value is: <code>true</code><br>
   * This should be set to </code>false</code> if the cacheable state of the child <code>Painter
   * </code>s are different from each other. This will allow the cacheable == <code>true</code>
   * <code>Painter</code>s to keep their cached image during regular repaints. In this case, client
   * code should call {@link #clearCache()} manually when the cacheable <code>Painter</code>s should
   * be updated.
   *
   * @see #isDirty()
   */
  ...
  /**
   * Set the flag used by {@link #isDirty()} to check if the child <code>Painter</code>s should be
   * checked for their <code>dirty</code> flag as part of processing.
   *
  ...
  /**
   * This <code>CompoundPainter</code> is dirty if it, or (optionally) any of its children, are
   * dirty. If the super implementation returns <code>true</code>, we return <code>true</code>.
   * Otherwise, if {@link #isCheckingDirtyChildPainters()} is <code>true</code>, we iterate over all
   * child <code>Painter</code>s and query them to see if they are dirty. If so, then <code>true
   * </code> is returned. Otherwise, we return <code>false</code>. {@inheritDoc} {@see
  ...
  /**
   * Clears the cache of this <code>Painter</code>, and all child <code>Painters</code>. This is
   * done to ensure that resources are collected, even if clearCache is called by some framework or
   * other code that doesn't realize this is a CompoundPainter.
   *
   * <p>Call #clearLocalCache if you only want to clear the cache of this <code>CompoundPainter
   * </code> {@inheritDoc}
   */
  @Override
