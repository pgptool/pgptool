...
package org.jdesktop.swingx.imported;
...
/**
 * <p>
 * A {@link javax.swing.JLabel} subclass which supports
 * {@link org.jdesktop.swingx.painter.Painter}s, multi-line text, and text
 * rotation.
 * </p>
 *
 * <p>
 * Painter support consists of the <code>foregroundPainter</code> and
 * <code>backgroundPainter</code> properties. The <code>backgroundPainter</code>
 * refers to a painter responsible for painting <i>beneath</i> the text and
 * icon. This painter, if set, will paint regardless of the <code>opaque</code>
 * property. If the background painter does not fully paint each pixel, then you
 * should make sure the <code>opaque</code> property is set to false.
 * </p>
 *
 * <p>
 * The <code>foregroundPainter</code> is responsible for painting the icon and
 * the text label. If no foregroundPainter is specified, then the look and feel
 * will paint the label. Note that if opaque is set to true and the look and
 * feel is rendering the foreground, then the foreground <i>may</i> paint over
 * the background. Most look and feels will paint a background when
 * <code>opaque</code> is true. To avoid this behavior, set <code>opaque</code>
 * to false.
 * </p>
 *
 * <p>
 * Since JXLabel is not opaque by default (<code>isOpaque()</code> returns
 * false), neither of these problems typically present themselves.
 * </p>
 *
 * <p>
 * Multi-line text is enabled via the <code>lineWrap</code> property. Simply set
 * it to true. By default, line wrapping occurs on word boundaries.
 * </p>
 *
 * <p>
 * The text (actually, the entire foreground and background) of the JXLabel may
 * be rotated. Set the <code>rotation</code> property to specify what the
 * rotation should be. Specify rotation angle in radian units.
 * </p>
 *
...
public class JXLabel extends JLabel {
	...
	/**
	 * Text alignment enums. Controls alignment of the text when line wrapping is
	 * enabled.
	 */
	public enum TextAlignment implements IValue {
		LEFT(StyleConstants.ALIGN_LEFT), CENTER(StyleConstants.ALIGN_CENTER), RIGHT(
				StyleConstants.ALIGN_RIGHT), JUSTIFY(StyleConstants.ALIGN_JUSTIFIED);
	...
	// using reverse logic ... some methods causing re-flow of text are called from
	// super constructor, but private variables are initialized only after call to
	// super so have to rely on default for boolean being false
	...
	/**
	 * Returns the current foregroundPainter. This is a bound property. By default
	 * the foregroundPainter will be an internal painter which executes the standard
	 * painting code (paintComponent()).
	 *
	...
	/**
	 * Sets a new foregroundPainter on the label. This will replace the existing
	 * foreground painter. Existing painters can be wrapped by using a
	 * CompoundPainter.
	 *
	 * @param painter
	 */
	...
	/**
	 * Sets a Painter to use to paint the background of this component By default
	 * there is already a single painter installed which draws the normal background
	 * for this component according to the current Look and Feel. Calling
	...
	/**
	 * Returns the current background painter. The default value of this property is
	 * a painter which draws the normal JPanel background according to the current
	 * look and feel.
	 *
	...
	public Dimension getPreferredSize() {
		...
		if (isPreferredSizeSet()) {
			...
			// #swingx-680 change the preferred size when rotation is set ... ideally this
			// would be solved in the LabelUI rather then here
			double theta = getTextRotation();
			size.setSize(rotateWidth(size, theta), rotateHeight(size, theta));
		} else {
			// #swingx-780 preferred size is not set properly when parent container doesn't
			// enforce the width
			View view = getWrappingView();
			if (view == null) {
				if (isLineWrap() && !MultiLineSupport.isHTML(getText())) {
					// view might get lost on LAF change ...
					putClientProperty(BasicHTML.propertyKey, getMultiLineSupport().createView(this));
			...
			int lsb = 0;
			/*
			 * Unless both text and icon are non-null, we effectively ignore the value of
			 * textIconGap.
			 */
			int gap;
			if (textIsEmpty) {
	 ...
	 * Note that although property name suggests only text rotation, the whole
	 * foreground painter is rotated in fact. Due to various reasons it is strongly
	 * discouraged to access any size related properties of the label from other
	 * threads then EDT when this property is set.
	 *
	...
	/**
	 * Enables line wrapping support for plain text. By default this support is
	 * disabled to mimic default of the JLabel. Value of this property has no effect
	 * on HTML text.
	 *
	 * @param b
	 *            the new value
	...
	/**
	 * Returns the current status of line wrap support. The default value of this
	 * property is false to mimic default JLabel behavior. Value of this property
	 * has no effect on HTML text.
	 *
	...
	/**
	 * Returns true if the background painter should paint where the border is or
	 * false if it should only paint inside the border. This property is true by
	 * default. This property affects the width, height, and intial transform passed
	...
	/**
	 * Sets the paintBorderInsets property. Set to true if the background painter
	 * should paint where the border is or false if it should only paint inside the
	 * border. This property is true by default. This property affects the width,
	 * height, and initial transform passed to the background painter.
	 *
	...
	protected void paintComponent(Graphics g) {
		// log.fine("in");
		// resizing the text view causes recursive callback to the paint down the road.
	...
	private Point2D calculateT() {
		...
		// orthogonal cases are most likely the most often used ones, so give them
		...
		if ((textRotation > 4.697 && textRotation < 4.727) || (textRotation > 1.555 && textRotation < 1.585)) {
			...
			// the rest of it. Calculate best rectangle that fits the bounds. "Best" is
			// considered one that
			// allows whole text to fit in, spanned on preferred axis (X). If that doesn't
			// work, fit the text
			// inside square with diagonal equal min(height, width) (Should be the largest
			...
			if (v == null) {
				...
				while (!ready) {
					...
					if (w < square || h > square) {
						// text is too long to fit no matter what. Revert shape to square since that is
						// the
						// best option (1st derivation for area size of rotated rect in rect is equal 0
						// for
	...
	// ----------------------------------------------------------
	// FYI: This class doesn't reinvent line wrapping. Instead it makes use of
	// existing support
	// made for JTextComponent/JEditorPane.
	// All the classes below named Alter* are verbatim copy of swing.text.* classes
	// made to
	// overcome package visibility of some of the code. All other classes here, when
	// their name
	// matches corresponding class from swing.text.* package are copy of the class
	// with removed
	// support for highlighting selection. In case this is ever merged back to JDK
	// all of this
	// can be safely removed as long as corresponding swing.text.* classes make
	// appropriate checks
	// before casting JComponent into JTextComponent to find out selected region
	// since
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (src.isLineWrap()) {
				if ("font".equals(name) || "foreground".equals(name) || "maxLineSpan".equals(name)
						|| "textAlignment".equals(name) || "icon".equals(name) || "iconTextGap".equals(name)) {
	...
	static class Renderer extends WrappedPlainView {
		...
		/**
		 * Fetches the attributes to use when rendering. At the root level there are no
		 * attributes. If an attribute is resolved up the view hierarchy this is the end
		 * of the line.
		 */
		@Override
		...
		/**
		 * Returns the number of views in this view. Since this view simply wraps the
		 * root of the view hierarchy it has exactly one child.
		 *
		...
		public float getPreferredSpan(int axis) {
			if (axis == X_AXIS) {
				// log.fine("inv: " + invalidated + ", w:" + width + ", vw:" +
				// host.getVisibleRect());
				...
				if (invalidated) {
					...
					if (w != 0) {
						...
						// JXLabelTest3 works; 20 == width of the parent border!!! ... why should this
						// screw with us?
		...
		/**
		 * Fetches the factory to be used for building the various view fragments that
		 * make up the view that represents the model. This is what determines how the
		 * model will be represented. This is implemented to fetch the factory provided
		 * by the associated EditorKit.
		 *
		 * @return the factory
		 */
		@Override
