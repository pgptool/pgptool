...
/**
 * A {@link javax.swing.JLabel} subclass which supports {@link
 * org.jdesktop.swingx.painter.Painter}s, multi-line text, and text rotation.
 *
 * <p>Painter support consists of the <code>foregroundPainter</code> and <code>backgroundPainter
 * </code> properties. The <code>backgroundPainter</code> refers to a painter responsible for
 * painting <i>beneath</i> the text and icon. This painter, if set, will paint regardless of the
 * <code>opaque</code> property. If the background painter does not fully paint each pixel, then you
 * should make sure the <code>opaque</code> property is set to false.
 *
 * <p>The <code>foregroundPainter</code> is responsible for painting the icon and the text label. If
 * no foregroundPainter is specified, then the look and feel will paint the label. Note that if
 * opaque is set to true and the look and feel is rendering the foreground, then the foreground
 * <i>may</i> paint over the background. Most look and feels will paint a background when <code>
 * opaque</code> is true. To avoid this behavior, set <code>opaque</code> to false.
 *
 * <p>Since JXLabel is not opaque by default (<code>isOpaque()</code> returns false), neither of
 * these problems typically present themselves.
 *
 * <p>Multi-line text is enabled via the <code>lineWrap</code> property. Simply set it to true. By
 * default, line wrapping occurs on word boundaries.
 *
 * <p>The text (actually, the entire foreground and background) of the JXLabel may be rotated. Set
 * the <code>rotation</code> property to specify what the rotation should be. Specify rotation angle
 * in radian units.
 *
...
public class JXLabel extends JLabel {
  ...
  /**
   * Returns the current foregroundPainter. This is a bound property. By default the
   * foregroundPainter will be an internal painter which executes the standard painting code
   * (paintComponent()).
   *
  ...
  /**
   * Sets a Painter to use to paint the background of this component By default there is already a
   * single painter installed which draws the normal background for this component according to the
   * current Look and Feel. Calling <CODE>setBackgroundPainter</CODE> will replace that existing
   * painter.
   *
  ...
  /**
   * Enables line wrapping support for plain text. By default this support is disabled to mimic
   * default of the JLabel. Value of this property has no effect on HTML text.
   *
  ...
  /**
   * Returns true if the background painter should paint where the border is or false if it should
   * only paint inside the border. This property is true by default. This property affects the
   * width, height, and intial transform passed to the background painter.
   *
   ...
   * default. This property affects the width, height, and initial transform passed to the
   * background painter.
   *
  ...
  private Point2D calculateT() {
    ...
    if ((textRotation > 4.697 && textRotation < 4.727)
        || (textRotation > 1.555 && textRotation < 1.585)) {
      // vertical
      int tmp = pHeight;
      pHeight = pWidth;
      pWidth = tmp;
      ...
      if (v == null) {
        ...
        double cw =
            (getWidth() - Math.abs(ty * Math.sin(textRotation))) / Math.abs(Math.cos(textRotation));
        double ch =
            (getHeight() - Math.abs(ty * Math.cos(textRotation)))
    ...
    public static View createView(JXLabel c) {
      ...
      Document doc =
          kit.createDefaultDocument(
              c.getFont(), c.getForeground(), c.getTextAlignment(), rightIndent);
  ...
  static class Renderer extends WrappedPlainView {
    ...
    /**
     * Fetches the attributes to use when rendering. At the root level there are no attributes. If
     * an attribute is resolved up the view hierarchy this is the end of the line.
     */
    @Override
    ...
    /**
     * Returns the number of views in this view. Since this view simply wraps the root of the view
     * hierarchy it has exactly one child.
     *
     ...
     * finding out the host components font, etc. The default implementation of this is to forward
     * the query to the parent view.
     *
    ...
    /**
     * Fetches the factory to be used for building the various view fragments that make up the view
     * that represents the model. This is what determines how the model will be represented. This is
