 ...
 *
 * <p>Use {@link #create(String, ProgressHandler)} to instantiate it. Note that as a result instance
 * of {@link Updater} will be returned. You'll update progress through this helper class
 *
...
public class Progress {
  ...
  /**
   * Calling this method will set a flag that must be handled by executor. And once it's handled
   * {@link UserRequestedCancellationException} will be thrown
   */
  ...
  public class Updater {
    ...
    public void updateStepsTaken(BigInteger stepsTaken) {
      Preconditions.checkState(
          totalSteps != null && !BigInteger.ZERO.equals(totalSteps),
      ...
      percentage =
          isCompleted
              ? 100
              : stepsTaken.multiply(BigInteger.valueOf(100)).divide(totalSteps).intValue();
      updateEta();
      notifyHandler();
    }
    ...
    private void updateEta() {
      ...
      toBeFinishedBy =
          BigInteger.valueOf(elapsed).multiply(totalSteps).divide(stepsTaken).longValue();
    }
  }
}
