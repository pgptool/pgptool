...
public class MonitoringDecryptedFilesServiceImpl
  ...
  private void setupFileWatcher() {
    ...
    multipleFilesWatcher =
        new MultipleFilesWatcher(dirWatcherHandler, "MonitoringDecryptedFilesService");
  ...
  public synchronized void addOrUpdate(DecryptedFile decryptedFile) {
    try {
      ...
      Preconditions.checkArgument(
          decryptedFile.getDecryptedFile() != null, "decryptedFile.DecryptedFile is NULL");
      Preconditions.checkArgument(
          decryptedFile.getEncryptedFile() != null, "decryptedFile.EncryptedFile is NULL");
  ...
  public synchronized List<DecryptedFile> getDecryptedFiles() {
    return new ArrayList<>(
        monitoredDecrypted.getAll().stream()
            .map(x -> (DecryptedFile) x.getValue())
  ...
  public DecryptedFile findByDecryptedFile(String decryptedFile) {
    return getDecryptedFiles().stream()
        .filter(x -> x.getDecryptedFile().equals(decryptedFile))
        .findFirst()
        .orElse(null);
  }
  ...
  public DecryptedFile findByEncryptedFile(String encryptedFile, Predicate<DecryptedFile> filter) {
    return getDecryptedFiles().stream()
        .filter(x -> x.getEncryptedFile().equals(encryptedFile) && filter.test(x))
        .findFirst()
        .orElse(null);
  }
}
