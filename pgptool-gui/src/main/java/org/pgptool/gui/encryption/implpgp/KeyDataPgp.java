 ...
 * "ring" for public info. It's weird for naked eye.
 *
...
public class KeyDataPgp extends KeyData {
  ...
  public static KeyDataPgp cast(KeyData keyData) {
    ...
    Preconditions.checkArgument(
        keyData instanceof KeyDataPgp, "keyData is expected to be of type KeyDataPgp");
      ...
      if (ois.readBoolean()) {
        secretKeyRing =
            new PGPSecretKeyRing(
                initInputStream(ois), KeyFilesOperationsPgpImpl.fingerprintCalculator);
      }
      if (ois.readBoolean()) {
        publicKeyRing =
  ...
  /**
   * @return PGPPublicKeyRing or NULL if it's not separately defined. If it's NULL check secret key
   *     ring. It's might be NULL if only private key ring was imported without separate public ring
   */
