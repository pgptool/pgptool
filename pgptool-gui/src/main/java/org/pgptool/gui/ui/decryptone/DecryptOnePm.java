...
public class DecryptOnePm extends PresentationModelBaseEx<DecryptOneHost, String> {
  ...
  private void initModelProperties() {
    sourceFile =
        new ModelProperty<>(this, new ValueAdapterHolderImpl<>(), FN_SOURCE_FILE, validationErrors);
    sourceFile.getModelPropertyAccessor().addPropertyChangeListener(onSourceFileModified);

    isUseTempFolder =
        new ModelProperty<>(this, new ValueAdapterHolderImpl<>(true), "saveToTempFolder");
    isUseSameFolder =
    ...
    targetFileEnabled =
        new ModelProperty<>(this, new ValueAdapterHolderImpl<>(), "targetFileEnabled");
    isUseBrowseFolder
        .getModelPropertyAccessor()
        .addPropertyChangeListener(onUseBrowseFolderChanged);
    ...
    if (targetFileChooser == null) {
      targetFileChooser =
          new SaveFileChooserDialog(
              "action.chooseTargetFile", "action.choose", appProps, "DecryptionTargetChooser") {
            @Override
            ...
            protected void suggestTarget(JFileChooser ofd) {
              ...
              if (StringUtils.hasText(targetFile.getValue())) {
                ...
                if (decryptionDialogParameters.getSourceFile().equals(sourceFileStr)) {
                  ...
                  use(
                      ofd,
                      madeUpTargetFileName(
                          FilenameUtils.getFullPathNoEndSeparator(
                              decryptionDialogParameters.getTargetFile())));
                }
              } else if (StringUtils.hasText(sourceFileStr) && new File(sourceFileStr).exists()) {
                use(
                    ofd,
                    madeUpTargetFileName(FilenameUtils.getFullPathNoEndSeparator(sourceFileStr)));
              }
            }
  ...
  private final PropertyChangeListener onSourceFileModified =
      new PropertyChangeListener() {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
          ...
          try {
            ...
            if (keyAndPassword != null
              ...
              Message purpose =
                  new Message(
                      "phrase.needKeyToDecryptFile",
                      new Object[] {FilenameUtils.getName(sourceFileStr)});
              // Detour
              host.askUserForKeyAndPassword(
        ...
        private DecryptionDialogParameters findParamsBasedOnSourceFile(String sourceFile) {
          ...
          if (params == null) {
            params =
                decryptionParams.find(FilenameUtils.getFullPathNoEndSeparator(sourceFile), null);
          }
          return params;
        }
  ...
  private class OperationWorker implements Runnable {
    ...
    public void run() {
      ...
      addToMonitoredDecrypted(
          sourceFileStr, targetFileName, sourceFileFingerprint, targetFileFingerprint);

      // Delete source if asked
      if (isDeleteSourceAfter.getValue()) {
        try {
          Preconditions.checkState(
              new File(sourceFileStr).delete(), "File.delete() returned false");
        } catch (Throwable t) {
          EntryPoint.reportExceptionToUser(
              workerOriginEvent, "error.decryptOkButCantDeleteSource", t);
        }
      }
      ...
      if (confirmationMessageRequired) {
        UiUtils.messageBox(
            workerOriginEvent,
            text("phrase.decryptionSuccess", targetFileName),
            text("term.success"),
    ...
    private void openAssociatedApp(String targetFileName) {
      try {
        ...
        EntryPoint.reportExceptionToUser(
            workerOriginEvent, "error.decryptOkButFailedToOpenAssociatedApp", t);
      }
    }
    ...
    private void browseForFolder(String targetFileName) {
      try {
        ...
        EntryPoint.reportExceptionToUser(
            workerOriginEvent, "error.decryptOkButCantBrowseForFolder", t);
      }
    }
    ...
    private String getEffectiveFileNameForTempFolder() {
      DecryptedFile dfm =
          monitoringDecryptedFilesService.findByEncryptedFile(
              getSourceFile().getValue(),
      ...
      String msg =
          text(
              "warning.fileWasAlreadyDecryptedIntoTempFolder",
              new Object[] {dfm.getEncryptedFile(), dfm.getDecryptedFile()});
      int response =
          JOptionPane.showConfirmDialog(
              findRegisteredWindowIfAny(),
              UiUtils.getMultilineMessage(msg),
              text("term.confirmation"),
     ...
     *
     * <p>NOTE: Slight SRP concern here. We're interfering with responsibility area of {@link
     * EncryptOnePm}
     *
