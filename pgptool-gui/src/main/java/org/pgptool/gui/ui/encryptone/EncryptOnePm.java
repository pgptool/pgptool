...
public class EncryptOnePm extends PresentationModelBaseEx<EncryptOneHost, String> {
    ...
    if (targetFileChooser == null) {
      targetFileChooser =
          new SaveFileChooserDialog(
              "action.chooseTargetFile", "action.choose", appProps, "EncryptionTargetChooser") {
            @Override
            ...
            protected void suggestTarget(JFileChooser ofd) {
              ...
              if (StringUtils.hasText(targetFile.getValue())) {
                ...
                if (encryptionDialogParameters.getSourceFile().equals(sourceFile.getValue())) {
                  ...
                  use(
                      ofd,
                      makeUpTargetFileName(
                          sourceFile.getValue(),
                          FilenameUtils.getFullPathNoEndSeparator(
                              encryptionDialogParameters.getTargetFile())));
                }
            ...
            private void use(JFileChooser ofd, String filePathName) {
              ofd.setCurrentDirectory(
                  new File(FilenameUtils.getFullPathNoEndSeparator(filePathName)));
  ...
  private void initModelProperties() {
    ...
    isUseSameFolder =
        new ModelProperty<>(this, new ValueAdapterHolderImpl<>(true), "saveToSameFolder");
  ...
  public ExistingFileChooserDialog getSourceFileChooser() {
    if (sourceFileChooser == null) {
      sourceFileChooser =
          new ExistingFileChooserDialog(appProps, SOURCE_FOLDER) {
            @Override
  ...
  private final PropertyChangeListener onSourceFileModified =
      new PropertyChangeListener() {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
          ...
          encryptionDialogParameters =
              encryptionParamsStorage.findParamsBasedOnSourceFile(sourceFile.getValue(), true);
        ...
        private void useSugestedParameters(
            EncryptionDialogParameters params, PropertyChangeEvent evt) {
          ...
          Set<String> missedKeys =
              preselectRecipients(new HashSet<>(params.getRecipientsKeysIds()));
  ...
  private class OperationWorker implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        if (isEncryptedFileChangedAfterDecryption(
                targetFileName, sourceFile.getValue(), progressHandler)
      ...
      if (isDeleteSourceAfter.getValue()) {
        try {
          Preconditions.checkState(
              new File(sourceFile.getValue()).delete(), "File.delete() returned false");
        } catch (Throwable t) {
          EntryPoint.reportExceptionToUser(
              workerOriginEvent, "error.encryptOkButCantDeleteSource", t);
        }
      } else {
        updateBaselineFingerprintsIfApplicable(
            sourceFile.getValue(), targetFileName, source, target);
      }
      ...
      encryptionParamsStorage.persistDialogParametersForCurrentInputs(
          encryptionDialogParameters, true);
    ...
    private boolean promptUserToOverwriteConcurrentChanges(String targetFileName) {
      return UiUtils.confirmRegular(
          workerOriginEvent,
          "confirm.doYouWantToOverwriteConcurrentChanges",
          new String[] {targetFileName});
    }

    private boolean isEncryptedFileChangedAfterDecryption(
        String encryptedFileName, String decryptedFilename, ProgressHandler progressHandler)
        throws IOException, UserRequestedCancellationException {
      File targetFileObj = new File(encryptedFileName);
      if (!targetFileObj.exists()) {
        return false;
      }

      DecryptedFile decryptedMonitored =
          monitoringDecryptedFilesService.findByDecryptedFile(decryptedFilename);
    ...
    private void askOperSystemToBrowseForFolder(String targetFileName) {
      try {
        ...
        EntryPoint.reportExceptionToUser(
            workerOriginEvent, "error.encryptOkButCantBrowseForFolder", t);
      }
    }
    ...
    private String getEffectiveTargetFileName() throws UserRequestedCancellationException {
      if (StringUtils.hasText(targetFile.getValue()) && !isUseSameFolder.getValue()) {
        ...
        Preconditions.checkState(
            parentFolder.exists() || parentFolder.mkdirs(),
      ...
      boolean confirmOverwriteWithUser =
          (encryptionDialogParameters == null
                  || !sourceFile
                      .getValue()
                      .equalsIgnoreCase(encryptionDialogParameters.getSourceFile()))
    ...
    private boolean promptUserToOverwriteExistingFile(String targetFileName) {
      return UiUtils.confirmWarning(
          workerOriginEvent, "confirm.overWriteExistingFile", new Object[] {targetFileName});
    }
  ...
  protected static String makeUpTargetFileName(String sourceFileName, String targetBasedPath) {
    ...
    return targetBasedPath
        + File.separator
        + fileNameOnlyWoPathAndExtension
        + "."
