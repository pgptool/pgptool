...
public class EncryptOneView extends DialogViewBaseEx<EncryptOnePm> {
  ...
  private JPanel buildControllsPanel() {
    ...
    JXLabel lblNoPrivateKeysSelectedWarning =
        new JXLabel(text("encryption.warningNoPrivateKeysSelected"));
    lblNoPrivateKeysSelectedWarning.setLineWrap(true);
    panelNoPrivateKeysSelectedWarning =
        wrapIntoMessagePanel(lblNoPrivateKeysSelectedWarning, new Color(221, 253, 191));
  ...
  protected void internalBindToPm() {
    ...
    isDisableControlsChanged.handlePropertyChanged(
        this, null, false, pm.getIsDisableControls().getValue());

    bindingContext.registerPropertyValuePropagation(pm.getProgressValue(), pbar, "value");

    bindingContext.registerOnChangeHandler(
        pm.getIsNoPrivateKeysSelected(), onNoPrivateKeysSelected);
    onNoPrivateKeysSelected.handlePropertyChanged(
        pm,
        pm.getIsNoPrivateKeysSelected().getPropertyName(),
        null,
  ...
  protected JDialog initDialog(Window owner, Object constraints) {
    ...
    ret.setMinimumSize(
        new Dimension(UiUtils.getFontRelativeSize(50), UiUtils.getFontRelativeSize(40)));
