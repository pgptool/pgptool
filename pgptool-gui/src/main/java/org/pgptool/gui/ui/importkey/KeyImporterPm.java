...
public class KeyImporterPm extends PresentationModelBaseEx<KeyImporterHost, List<Key>> {
  ...
  public MultipleFilesChooserDialog getSourceFileChooser() {
    if (sourceFileChooser == null) {
      sourceFileChooser =
          new MultipleFilesChooserDialog(appProps, BROWSE_FOLDER) {
            @Override
                  ...
                  public boolean accept(File f) {
                    ...
                    try {
                      ...
                      for (Key readKey : readKeys) {
                        Key existingKey =
                            keyRingService.findKeyById(readKey.getKeyInfo().getKeyId());
                        if (existingKey == null) {
                          return true;
                        }
  ...
  private String buildSummaryMessage(
      String messageCode, int successCount, Map<String, Throwable> exceptions) {
