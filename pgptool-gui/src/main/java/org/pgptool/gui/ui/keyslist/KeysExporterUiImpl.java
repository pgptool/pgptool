...
public class KeysExporterUiImpl implements KeysExporterUi {
  ...
  public void exportPublicKey(Key key, ActionEvent originEvent) {
    ...
    try {
      ...
      EntryPoint.reportExceptionToUser(
          originEvent, "error.failedToExportPublicKey", t, key.toString());
      return;
    }
  ...
  public SaveFileChooserDialog buildPublicKeyTargetChooser(Key key) {
    return new SaveFileChooserDialog(
        "action.exportPublicKey", "action.export", appProps, "ExportKeyDialog") {
      @Override
      ...
      protected void suggestTarget(JFileChooser ofd) {
        ...
        File suggestedFileName =
            suggestFileNameForKey(
                key, ofd.getCurrentDirectory().getAbsolutePath(), null, false, false);
  ...
  public void exportPrivateKey(Key key, ActionEvent originEvent) {
    ...
    try {
      ...
      EntryPoint.reportExceptionToUser(
          originEvent, "error.failedToExportPrivateKey", t, key.toString());
      return;
    }

    UiUtils.messageBox(
        originEvent,
        text("keys.privateKey.exportWarning"),
        text("term.attention"),
  ...
  public SaveFileChooserDialog buildPrivateKeyTargetChooser(final Key key) {
    return new SaveFileChooserDialog(
        "action.exportPrivateKey", "action.export", appProps, "ExportKeyDialog") {
      @Override
      protected void onFileChooserPostConstruct(JFileChooser ofd) {
        ofd.setAcceptAllFileFilterUsed(false);
        ofd.addChoosableFileFilter(new FileNameExtensionFilter("Key file armored (.asc)", "asc"));
      ...
      protected void suggestTarget(JFileChooser ofd) {
        ...
        File suggestedFileName =
            suggestFileNameForKey(
                key, ofd.getCurrentDirectory().getAbsolutePath(), " - private", false, false);
        ofd.setSelectedFile(suggestedFileName);
      }
    };
  }

  private File suggestFileNameForKey(
      Key key,
  ...
  private String addKeyIdIfFileAlreadyExists(
      Key key, boolean isAddExtension, String fileName, String fileNameWithoutExt) {
  ...
  public void exportPublicKeys(ArrayList<Key> keys, ActionEvent originEvent) {
    ...
    try {
      ...
      EntryPoint.reportExceptionToUser(
          originEvent, "error.failedToExportKeys", t, keysExported, keysTotal);
    } finally {
      if (keysExported > 0) {
