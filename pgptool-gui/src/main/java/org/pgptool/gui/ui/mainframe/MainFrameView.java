...
public class MainFrameView extends ViewBaseEx<MainFramePm> implements HasWindow {
  ...
  protected void internalBindToPm() {
    ...
    onHintPmChanged.handlePropertyChanged(
        pm, pm.getHintPm().getPropertyName(), null, pm.getHintPm().getValue());
  }
        ...
        public void handlePropertyChanged(
          ...
          tableColumnsGeometryPersister =
              new TableColumnsGeometryPersisterImpl(
                  table, "mfrmDecrMon", uiGeom, scheduledExecutorService);
  ...
  private void bindToActions() {
    ...
    bindingContext.registerPropertyValuePropagation(
        pm.getIsAutoUpdatesEnabled(), miAutoCheckForUpdates, "state");
    ...
    bindingContext.setupBinding(
        pm.getActionChangeFolderForDecrypted(), miChangeTempFolderForDecrypted);
  ...
  protected void internalRenderTo(Container owner, Object constraints) {
    ...
    if (frame == null) {
      ...
      frame.setMinimumSize(
          new Dimension(
              (int) (toolbar.getPreferredSize().getWidth() + UiUtils.getFontRelativeSize(2)),
      ...
      windowGeometryPersister =
          new WindowGeometryPersisterImpl(frame, "mainFrame", uiGeom, scheduledExecutorService);
