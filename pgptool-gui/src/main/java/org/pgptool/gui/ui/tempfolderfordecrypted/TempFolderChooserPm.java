...
/**
 * This simple presenter will provide user with ability to select other folder to store temporarily
 * decrypted files
 *
...
public class TempFolderChooserPm extends PresentationModelBaseEx<Void, Void> {
  ...
  public void present(ActionEvent originEvent) {
    try {
      ...
      UiUtils.messageBox(
          originEvent,
          text("phrase.settingsChangedConfirmFolder", newFolder),
          text("term.success"),
  ...
  public FolderChooserDialog getFolderChooserDialog() {
    if (folderChooserDialog == null) {
      ...
      folderChooserDialog =
          new FolderChooserDialog(text("term.changeTempFolderForDecrypted"), recentlyUsedFolder);
    }
