...
public class SubclassAwareJsonSerializationAdapter<T> implements JsonSerializer<T>, JsonDeserializer<T> {
	...
	/**
	 * We need that to avoid infinite recursion (stack overflow) when working with
	 * objects of class we have adapter for. If we reuse context to work with
	 * instances - it will fail. So we need separata Gson which is unaware of this
	 * custom SubclassAwareJsonSerializationAdapter
	 */
