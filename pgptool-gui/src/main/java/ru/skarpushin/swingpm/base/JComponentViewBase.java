...
/**
 * Abstract impl of view that extends from JComponent
 *
 * <p>NOTE: I hate that fact that I had to copy-paste {@link ViewBase} only because I need a view
 * that extends JComponent. Need to find a better approach so that I don't have to duplicate that
 * much code
 */
public abstract class JComponentViewBase<TPM extends PresentationModel> extends JComponent
    implements View<TPM>, InitializingBean {
  private static final org.slf4j.Logger log = LoggerFactory.getLogger(JComponentViewBase.class);
  ...
  protected Runnable buildInitComponentsRunnable() {
    return () -> {
      try {
        log.debug("Initializing components: {}", JComponentViewBase.this);
        internalInitComponents();
        log.debug("Initializing components - complete: {}", JComponentViewBase.this);
      } catch (Throwable t) {
  ...
  protected void internalSetPm(TPM newPm) {
    ...
    if (pm != null) {
      log.debug("Binding to PM...: {}", JComponentViewBase.this);
      internalBindToPm();
      log.debug("Binding to PM...Complete: {}", JComponentViewBase.this);
    }
  }
